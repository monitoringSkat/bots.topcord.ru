...
const getToken: Command = {
    ...
    async execute(client, message, [ id ]) {
        ...
        if (!bot) return message.channel.send(JSON.stringify(new BotNotFoundException(), null, 2))
        if (!(bot.owner.id === authorId)) return message.channel.send(JSON.stringify(new PermissionsDenied()))
        const token = sign({ botId: bot.id }, "secret-key", {expiresIn: "365days"})
